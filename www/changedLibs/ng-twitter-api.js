...
.factory('$twitterApi', ['$q', '$twitterHelpers', '$http', function($q, $twitterHelpers, $http) {
  ...
  function postPhoto(media_data) {
    var url = 'https://upload.twitter.com/1.1/media/upload.json';
    var deferred = $q.defer();

    // // Append the bodyparams to the URL
    var parameters = {media_data: media_data};
    var t = $twitterHelpers.createTwitterSignature('POST', url, {}, clientId, clientSecret, token);
    // url = url + '?media_data=true';
    // $http.defaults.headers.common.Content-Type = 'application/octet-stream';
    // $http.defaults.headers.common.Content-Transfer-Encoding = 'base64';
    ...
    //   method: 'POST',
    //   url: url,
    //   headers: 'application/octet-stream, base64',
    //   data: parameters
    // };
